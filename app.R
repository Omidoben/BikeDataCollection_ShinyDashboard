...
most_recent_bike_data <- function() {
  con <- dbConnect(
    MySQL(),
  ...
  query <- "SELECT * FROM latest_bike_stations
    WHERE time = (SELECT MAX(time) FROM latest_bike_stations)
    ORDER BY time DESC LIMIT 783"
